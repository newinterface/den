...
import {currencySelector, configsSelector} from 'src/modules/common/selectors';
...
class CartScreen extends React.Component {
  ...
  render() {
    const {t, navigation, data, totals, loading, currency, webviewCheckout} =
      this.props;
    return (
      <ThemeConsumer>
        {({theme}) => (
...
const mapStateToProps = state => {
  return {
    ...
    webviewCheckout: configsSelector(state).get('webviewCheckout', true),
  };
};
