...
/**
 * Fetch product wishlist data
 * @returns {*}
 */

export const getWishListProducts = (query, options = {}) => {
  const url = '/wc/v3/products';
  return request.get(
    `${url}?${queryString.stringify(
      pickBy(query, item => item !== ''),
...
export const getRatingProductReviews = product_id =>
  request.get(`/mobile-builder/v1/rating-count?product_id=${product_id}`);
...
export const addProductReviews = data =>
  request.post('/mobile-builder/v1/reviews', data);
